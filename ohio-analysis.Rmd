...
prop_file = file.path(main_folder, "opt.properties")
...
for (p in free_param_names) {
  ...
  dev.copy(pdf, file.path(plotting_folder, paste("2-", p, "-bootstrap_values.pdf", sep="")))
  dev.off()
...
NSIM <- 10000
K <- 30
B <- K-1  # bootstrap resamples
J <- 3

lik_mc <- function(phi,sigma,y,s,J,log=F){
 lik <- vector("numeric",length(y))
 for(n in 1:length(y)){
   set.seed(s+n-1)
   X <- rnorm(J,mean=phi,sd=sigma)
   fYgivenX <- dlnorm(y[n],meanlog=X,sdlog=sigma)
   lik[n] <- mean(fYgivenX)
 }
 if(!log) prod(lik) else sum(log(lik))
}

negloglik_mc_vec <- function(params,y,s,J){
 -lik_mc(phi=params["phi"],sigma=params["sigma"],y=y,s=s,J=J,log=T)
}

toy_bootstrap_mc <- function(Ydata,seed,confidence=0.95){
  mle <- optim(par=c(sigma=2,phi=1),fn=negloglik_mc_vec, method="Nelder-Mead",y=Ydata,s=seed,J=J)
  bootstrap_params <- matrix(0,nrow=B,ncol=2,dimnames=list(NULL,c("phi","sigma")))
  for(b in 1:B){
    set.seed(seed+b)
    bootstrap_sim <- rlnorm(N,meanlog=mle$par["phi"],sdlog=sqrt(2)*mle$par["sigma"])
    bootstrap_est <- optim(par=mle$par,fn=negloglik_mc_vec,
      method="Nelder-Mead",y=bootstrap_sim,s=seed+N*b,J=J)
    bootstrap_params[b,"sigma"] <- bootstrap_est$par["sigma"] 
    bootstrap_params[b,"phi"] <- bootstrap_est$par["phi"]
  }
  ci_sigma <- unname(quantile(bootstrap_params[,"sigma"],c((1-confidence)/2,(1+confidence)/2)))
  ci_phi <- unname(quantile(bootstrap_params[,"phi"],c((1-confidence)/2,(1+confidence)/2)))
  return(list(mle=mle$par,mle_value=-mle$value,
    #bootstrap_params=bootstrap_params,
    ci_sigma=ci_sigma,ci_phi=ci_phi))
}

library(foreach)
library(doParallel)
registerDoParallel(NCORES)
seed_simstudy_sim <- SEED_SIM
seed_simstudy_bootstrap <- SEED_EST
Nsim <- NSIM


negloglik_mc_vec <- mle_params$loglik
bs_ci_sim <- foreach(nsim=1:Nsim,.combine=rbind) %dopar% {
  set.seed(seed_simstudy_sim+nsim)
  Ysim <- mle_params[["w"]]
  bs1 <- toy_bootstrap_mc(Ysim,seed=seed_simstudy_bootstrap+nsim*N*(B+1),confidence=0.95)
  c(phi_lo=bs1$ci_phi[1],phi_hi=bs1$ci_phi[2],sigma_lo=bs1$ci_sigma[1],sigma_hi=bs1$ci_sigma[2])
}
stopImplicitCluster()
```
...
if (calculate_mcap_cis) {
  ...
  for (i in 1:nrow(free_param_box)) {
    ...
    for (val in par_range) {
      ...
      if (is.element(name, logit_trans_params) && transformVals) {
        logitVal <- logitTransform(val)
        #par_values <- c(par_values, val)
        par_values <- c(par_values, logitVal)
      } else if (is.element(name, log_trans_params) && transformVals) {
    ...
    par_range <- par_values
    
    #print(data.frame(par_range, par_values, log_likelihoods))
    ...
    x <- mcap(par_prof$log_likelihoods, par_prof$par_range, mcap_confidence, 0.75, mcap_ngrid[[i]])
    ...
    if (is.element(name, logit_trans_params) && transformVals) {
      print(sprintf("%s %0.2f %0.2f", name, sigmoidTransform(x$ci[1]), sigmoidTransform(x$ci[2])))
      var_label <- paste("logit(", name, ")")
    } else if (is.element(name, log_trans_params) && transformVals) {
      print(sprintf("%s %0.2f %0.2f", name, expTransform(x$ci[1]), expTransform(x$ci[2])))
    ...
    #par_prof_orig <- data.frame(par_range, log_likelihoods)
    #print(par_prof_orig)
